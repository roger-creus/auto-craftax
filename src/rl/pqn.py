...
from src.models.models import PQN_Agent
...
if __name__ == "__main__":
    ...
    run_name = f"algo:PQN_env:{args.env_id}_seed:{args.seed}_{int(time.time())}"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                if t == args.num_steps - 1:
                    next_value, _ = torch.max(agent(next_obs), dim=-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                old_val = agent(b_obs[mb_inds]).gather(1, b_actions[mb_inds].unsqueeze(-1).long()).squeeze()
    ...
    save_results_csv(args, "PQN", avg_ep_reward, avg_ep_length, avg_achievements, global_step, time.time() - start_time)